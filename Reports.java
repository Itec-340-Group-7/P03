...
public class Reports {
    ...
    public void financialDetail() throws SQLException {
        String format = "|%1$-16s|%2$-10s|%3$-10s|%4$-17s|\n";
        String condoDetailBanner =
                "*********************\n"
                        + "********************\n"
                        + "**Financial Detail**\n"
        ...
        for (int i = 1; i <= countTrips(); i++) {
            String query1 =
                            "SELECT t.resort, Sum(p.payment) AS Paid, Count(ca.RID) roomCount "
                            + "FROM SkiClub sc "
                            + "inner join Condo_Assign ca on sc.MID = ca.MID "
                            + "inner join Payment p on ca.mid = p.mid "
                            + "inner join Condo_Reservation cr on p.RID = cr.RId "
            ...
            ResultSet rset1 = stmt1.executeQuery(query1);
            String table = "TRIP " + i + "\n"
                    + String.format(format, "Resort Name", "Total Owed", "Total Paid", "Remaining Balance")
                    + String.format(format, "----------------", "----------", "----------", "-----------------");

            while (rset1.next()) {
                table += String.format(format, rset1.getString("Resort"),rset1.getInt("roomCount") * 100, rset1.getInt("Paid"), rset1.getInt("roomCount") * 100 - rset1.getInt("Paid"));
